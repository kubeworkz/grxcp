...
if [[ -n "$GRXGPU" && -d "$TOOLDIR/llvm-vortex" ]]; then
  ...
  # grxblas.o too: grxdnnAttentionForward calls grxBLAS, so libgrxdnn does not
  # link on its own any more. The grxBLAS gate above built it.
  $CXX "${OBJS[@]}" "$BUILD/grxblas.o" "$BUILD/grxdnn.o" "$BUILD/test_grxdnn.o" \
    $LIBS -o "$BUILD/test_grxdnn"
...
# transposed -- so it is almost entirely index bookkeeping, and a reference
# written from the same reasoning as the implementation would agree with it
# whether or not either was right.
#
# The expected values therefore come from torch.nn.functional.scaled_dot_
# product_attention, in float64, generated by tests/libs/attention_ref.py and
# checked in as attention_ref.bin so this gate needs neither Python nor torch.
# That script also simulates the exact grxblasSgemm calls this makes, on flat
# memory, and refuses to write the vectors unless the simulation reproduces
# torch to 1e-12 -- so the layout algebra is settled before a device sees it.
if [[ -n "$GRXGPU" && -d "$TOOLDIR/llvm-vortex" ]]; then
  $CXX $CXXFLAGS -I"$ROOT/tests/unit" -c "$ROOT/tests/libs/test_grxdnn_attn.cpp" \
    -o "$BUILD/test_grxdnn_attn.o"
  $CXX "${OBJS[@]}" "$BUILD/grxblas.o" "$BUILD/grxdnn.o" \
...
if [[ -n "$GRXGPU" && -d "$TOOLDIR/llvm-vortex" ]]; then
  ...
  "$ROOT/ci/build_kernel.sh" --grxgpu "$GRXGPU" --tooldir "$TOOLDIR" \
    "$ROOT/src/libs/grxdnn/kernels/all.cpp" \
...
# transpose decision out of the k loop -- and is now 3.65x faster at these
# shapes, so the ratio fell from 5.62x to 2.69x with the tensor path completely
# unchanged (29.9 and 44.4 cycles per element either way). The
