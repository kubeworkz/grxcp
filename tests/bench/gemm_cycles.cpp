...
int main(int argc, char** argv) {
  ...
  if (have_tensor) {
    ...
    if (worst_ratio == 0.0) {
      ...
      // loop -- and the tuned SIMT kernel is now 3.76x faster than the
      // reference at these shapes. The tensor path did not move: 29.6 and 44.4
      // cycles per element in both configurations, which is why the line above
      // is printed. The ratio has fallen from 5.62x to 2.65x with nothing
      // having got slower.
      //
